...
public interface DetallePedidoRepository extends PagingAndSortingRepository<DetallePedido, Integer> {
    ...
    @Query(value="SELECT sum(dp.precio_unitario*dp.cantidad) AS TOTAL, pe.fecha_entrega AS fechaCreacion FROM pm_c_detallepedido dp INNER JOIN pm_c_pedido pe ON pe.id = dp.id_pedido WHERE pe.fecha_entrega BETWEEN :from AND :to GROUP BY date_format(pe.fecha_entrega, \"%d-%m-%Y\")", nativeQuery = true)
